...
char *intToGchar(int i) { return g_strdup_printf("%i", i); }
...
GtkWidget *getWidget(GtkBuilder *builder, gchar *id) { return GTK_WIDGET(gtk_builder_get_object(builder, id)); }
...
int *getGamesList2(char *dir) {
    // fclose(fp);
    getValueFromJSON(dir, "InstallConfigStore/Software/Valve/Steam/ShaderCacheManager/CommittedDepotManifests");
    // GError *error = NULL;
...
const char *getGameNameFromID(char *appID) {
    ...
    if (curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_URL, addChar("https://store.steampowered.com/api/appdetails/?appids=", appID));
        ...
        JsonArray *results = json_node_get_array(json_path_query(path, json_from_string(req.buffer, &error), &error));
        if (error != NULL) {
            fprintf(stderr, "%s", error->message);
            free(req.buffer);
            exit(1);
